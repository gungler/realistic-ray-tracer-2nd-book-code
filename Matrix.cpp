...
Matrix viewMatrix(const Vector3& eye, const Vector3& gaze,
                        const Vector3& up){
    Matrix ret = identityMatrix();
    //creates an orthoganal basis from parameters
    Vector3 w = -(unitVector(gaze));
    Vector3 u = unitVetcor(cross(up, w));
    Vector3 v = cross(w, u);

    //creates an orthoganal basis to xyz basis
    ret.x[0][0] = u.x();
    ret.x[0][1] = u.y();
    ret.x[0][2] = u.z();
    ret.x[1][0] = v.x();
