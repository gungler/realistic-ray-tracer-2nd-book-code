...
Parallelogram::Parallelogram(const Vector3& _base, const vector& _u, const Vector3& _v,
        Material* _mptr)base(_base), u(_u), v(_v){
...
Parallelogram::Parallelogram(const Vector3& _base, const Vector3& _u, const Vector3& _v
        const Vector2& _uv0, const Vector2& _uv1, const Vector2& _uv2,
        Material* _mptr):base(_base), u(_u), v(_v), uv0(_uv0), uv1(_uv1), uv2(_uv2){
...
bool Parallelogram::shadowHit(const Ray& r, float tmin, float tmax,
        float time) const{
    
    double dot1 = dot(r.direction(), norm);

    //check to see if ray is parallel to surface plane
    if(dit1 < PARALLEL_EPSILON && dot1 > -PARALLEL_EPSILON) return false;

    //find distance to surfaceplane
    float dot2 = dot(norm, base);
    float t = (dot2 - dot(norm, r.origin()))/dot1;
    if(t > tmax || t < tmin) return false;

    //see if point of plane intersectiion is within parallelogram
    Vector3 hit_plane(r.origin()+r.direction() * t);
    Vector3 offset(hit_plane - base);
...
BBox Parallelogram::boudingBox(float time0, float time1) const{
    ...
    max.setX(p2.x() > max.x()? p2.x():min.x());
    max.setY(p2.y() > max.y()? p2.y():min.y());
    max.setZ(p2.z() > max.z()? p2.z():min.z());

    max.setX(p3.x() > max.x()? p3.x():min.x());
    max.setY(p3.y() > max.y()? p3.y():min.y());
    max.setZ(p3.z() > max.z()? p3.z():min.z());
...
bool Parallelogram::randomPoint(const Vector3& viewpoint, const Vector2& seed, float time,
