...
bool MeshTriangle::hit(const Ray& r, float tmin, float tmax,
                        float time, HitRecord& rec) const{
    ...
    float denim = (A*EIHF + B*GFDI + C*DHEG);
    ...
    tval = -(F*AKJB + E*JCAL + D*BLCK)/denom;
...
bool MeshTriangle::shadowHit(conts Ray& r, float tmin, float tmax,
                                float time) const{
        Vector3 p0((mesh_ptr->vertUVs[p[0]]).vertex);
    Vector3 p1((mesh_ptr->vertUVs[p[1]]).vertex);
    Vector3 p2((mesh_ptr->vertUVs[p[2]]).vertex);
